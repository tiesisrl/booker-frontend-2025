...
const discountCode = ref("TEST10P");
...
<template>
    <div id="booking-page" class="page page-content pb-8">
        ...
        <div class="container grid pt-6 grid">
            <!-- <div class="grid">
                <div :class="index === 0 ? `col-12` : `col-4`" v-for="(event, index) in eventDates" :key="event.uuid">
                    <BEventCard :event="event" :list="index === 0" />
                </div>
            </div> -->
            <Stepper linear v-model:activeStep="active">
                <StepperPanel>
                    <template #content="{ nextCallback }">
                        <div v-for="(event, index) in eventDates" :key="event.uuid" list>
                            <BEventCard :event="event" @clicked="() => {
                                selectedEvent = event;
                                nextCallback(event);
                            }" />
                        </div>
            ...
            />
                ...
                <div class="col text-center">
                    <Button label="Associati ad una prenotazione esistente" class="text-xl border-round-xl text-white"
                        :pt="bookTypeButtonPT" @click="showAssociateDialog = true;" size="large" />
                </div>
            </div>
            <Stepper linear v-model:activeStep="active">
                <StepperPanel>
            ...
            <div>
                <div><b>selectedEvent</b>: {{ selectedEvent }}</div>
                <div><b>selectedTimeslot</b>: {{ selectedTimeslot }}</div>
                <div><b>selectedOption</b>: {{ selectedOption }}</div>
                <div><b>selectedProduct</b>: {{ selectedProduct }}</div>
            </div>
